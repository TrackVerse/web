...
Appears under the "Features" header, `Movies` subheader in a potential changelog:

```
feat(Movies): add rewatched status
```

Appears under the "Bug Fixes" header, `Auth` subheader, with a link to issue #41 in a potential changelog:
...
```
perf(API): reduce media list query time

BREAKING CHANGE: Pagination parameters were changed.
```
...
The scope could be anything specifying the place of the commit change, usually the name of the tab. For example `Auth`, `Games`, `API` (Application Programming Interface), `Tracker` etc...
